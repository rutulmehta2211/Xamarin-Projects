...
using static ERecall.ServiceLayer.ServiceModel.GetLatestProducts_Feeds;

namespace ERecall.Views.FeedsModule
{
    [XamlCompilation(XamlCompilationOptions.Compile)]
    public partial class LatestProducts : ContentPage
    {
        int index = 0;
        ObservableCollection<Response> objLstResponse;

        public static bool IsLoadBefore = false;
        public LatestProducts()
        {
            InitializeComponent();
            IsLoadBefore = false;
            lstLatestProducts.ItemSelected += (sender, e) =>
            {
        ...
        protected async override void OnAppearing()
        {
            ...
            if (!IsLoadBefore)
            {
                ...
                if (CheckInternetConnection.IsInternetConnected())
                {
                    ...
                    await GetService_GetLatestProducts();
        ...
        {
            lstLatestProducts.IsVisible = !IsVisible;
            lblNoRecordFound.IsVisible = IsVisible;
        }

        private async Task GetService_GetLatestProducts()
        {
            try
            {
                string url = ServiceURLs.GetLatestProducts + "PageIndex=" + index + "&PageSize=5";
                var Service_response = await GetResponseFromWebService.GetResponse<GetLatestProducts_Feeds.RootObject>(url);
                if (Service_response != null)
                {
                    if (Service_response.status)
                    {
                        if (Service_response.response != null && Service_response.response.Count > 0)
                        {
                            ...
                            lstLatestProducts.ItemsSource = objLstResponse;
                            if (index == 0)
                            {
                                lstLatestProducts.ScrollTo(objLstResponse[0], ScrollToPosition.Start, true);
                            }
                        }
                    }
                    else
        ...
        {
            lstLatestProducts.IsPullToRefreshEnabled = false;
            index = 0;
            objLstResponse = null;
            IsLoadBefore = false;
            if (CheckInternetConnection.IsInternetConnected())
            ...
            lstLatestProducts.IsRefreshing = false;
            lstLatestProducts.IsPullToRefreshEnabled = true;
        }
