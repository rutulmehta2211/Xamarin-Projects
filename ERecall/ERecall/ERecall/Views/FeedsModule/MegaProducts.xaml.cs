...
using static ERecall.ServiceLayer.ServiceModel.GetMegaProducts_Feeds;

namespace ERecall.Views.FeedsModule
{
    [XamlCompilation(XamlCompilationOptions.Compile)]
    public partial class MegaProducts : ContentPage
    {
        int index = 0;
        ...
        {
            InitializeComponent();
            lstMegaProducts.ItemSelected += (sender, e) =>
            {
        ...
        protected async override void OnAppearing()
        {
            ...
            if (CheckInternetConnection.IsInternetConnected())
            {
                ...
                await GetService_GetMegaProducts();
        ...
        {
            lstMegaProducts.IsVisible = !IsVisible;
            lblNoRecordFound.IsVisible = IsVisible;
        }

        private async Task GetService_GetMegaProducts()
        {
            try
            {
                string url = ServiceURLs.GetMegaProducts + "PageIndex=" + index + "&PageSize=5";
                var Service_response = await GetResponseFromWebService.GetResponse<GetMegaProducts_Feeds.RootObject>(url);
                if (Service_response != null)
                {
                    if (Service_response.status)
                    {
                        if (Service_response.response != null && Service_response.response.Count > 0)
                        {
                            ...
                            lstMegaProducts.ItemsSource = objLstResponse;
                            if (index == 0)
                            {
                                lstMegaProducts.ScrollTo(objLstResponse[0], ScrollToPosition.Start, true);
                            }
                        }
                    }
                    else
        ...
        {
            lstMegaProducts.IsPullToRefreshEnabled = false;
            index = 0;
            objLstResponse = null;
            if (CheckInternetConnection.IsInternetConnected())
            {
            ...
            lstMegaProducts.IsRefreshing = false;
            lstMegaProducts.IsPullToRefreshEnabled = true;
        }
